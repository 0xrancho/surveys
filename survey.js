...
Respond with ONLY one of these four words: crystal, few_ideas, need_think, no_idea`;
  ...
  async function showResult() {
    ...
    const [aiUnlockResult, imageResult] = await Promise.allSettled([
      fetchAIUnlockAsync(avatarOutput?.meta?.aiUnlockPrompt),
      fetchAvatarImageAsync(avatarOutput?.meta?.imagePrompt)
    ]);
    console.log('✅ API calls complete:', { aiUnlock: aiUnlockResult.status, image: imageResult.status });
    ...
    document.getElementById('avatarIdentity').textContent = avatarOutput?.avatar?.identity || personaConfig.description;
    document.getElementById('avatarStrength').textContent = avatarOutput?.avatar?.strength || personaConfig.traits.join('. ');
  ...
  async function submitSurveyData(persona, avatarOutput) {
    ...
    const fields = {
      ...
      'Avatar Identity': avatarOutput?.avatar?.identity || '',
      'Avatar Strength': avatarOutput?.avatar?.strength || '',
